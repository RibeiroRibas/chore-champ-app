...
import '../components/delete_reward_dialog_component.dart';
import '../components/reward_card_component.dart';
import '../components/reward_form_dialog_component.dart';
...
class _RewardsPageState extends ConsumerState<RewardsPage> {
  ...
  void _handleSave() {
    ...
    if (_editingReward != null) {
      ref.read(rewardsProvider.notifier).updateReward(
            _editingReward!.id,
              ...
              children: [
                  ...
                  child: Column(
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          Text(AppStrings.rewards, style: Theme.of(context).textTheme.titleLarge),
                          if (isAdmin)
                            TextButton.icon(
                      ...
                      if (rewards.isEmpty)
                        Center(
                          child: Padding(
                            ...
                            child: Text(
                              '${AppStrings.noRewardsYet} ${isAdmin ? AppStrings.createFirstReward : AppStrings.askAdminToAddRewards}',
                        ...
                        ...rewards.map((reward) {
                          ...
                          try {
                            achievement = achievements.firstWhere((a) => a.id == reward.achievementId);
                          } catch (_) {}
                          final achievementUnlocked = achievement?.unlockedBy.contains(currentUser.id) ?? false;
                          final claimed = reward.claimedBy.contains(currentUser.id);
                          ...
                          return RewardCardComponent(
                            ...
                            onDelete: () => setState(() => _deleteId = reward.id),
                            onClaim: () => ref.read(rewardsProvider.notifier).claimReward(reward.id, currentUser.id),
                          );
                        }),
                ...
                if (_dialogOpen)
                  RewardFormDialogComponent(
                    ...
                    onAchievementIdChanged: (v) => setState(() => _achievementId = v ?? ''),
                    onCancel: () => setState(() => _dialogOpen = false),
                    onSave: _handleSave,
                    canSave: _title.trim().isNotEmpty && _achievementId.isNotEmpty,
                  ),
                if (_deleteId != null)
